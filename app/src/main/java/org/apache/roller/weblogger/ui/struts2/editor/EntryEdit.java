...
import org.apache.roller.weblogger.business.feed.FeedProcessingPipeline;
...
import org.apache.roller.weblogger.business.AuthorizationServiceImpl;
...
public final class EntryEdit extends UIAction {
    ...
    public String publish() {
        ...
        if (authorizationService.hasUserPermission(
                getAuthenticatedUser(), getActionWeblog(), WeblogPermission.POST)) {
    ...
    private String save() {
        if (!hasActionErrors()) {
            try {
                ...
                // Run the feed processing pipeline (Chain of Responsibility):
                // profanity filter → word-count limiter → auto-tag generation.
                // Steps are independently enabled/disabled via roller-custom.properties.
                FeedProcessingPipeline.getInstance().process(weblogEntry);
    ...
    public boolean isUserAnAuthor() {
        AuthorizationServiceImpl authorizationService = new AuthorizationServiceImpl();
        return authorizationService.hasUserPermission(getAuthenticatedUser(), getActionWeblog(),
