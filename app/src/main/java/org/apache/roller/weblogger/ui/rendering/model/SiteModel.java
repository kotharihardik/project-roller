...
public class SiteModel implements Model {
    ...
    public List<StatCount> getTrendingWeblogEntries(int length) {
        try {
            StarService starService = WebloggerFactory.getWeblogger().getStarService();
            return starService.getTopStarredEntries(0, length);
        } catch (Exception e) {
            log.error("ERROR: fetching trending weblog entries", e);
        }
        return Collections.emptyList();
    }
    ...
    public List<StatCount> getTrendingWeblogs(int length) {
        try {
            StarService starService = WebloggerFactory.getWeblogger().getStarService();
            return starService.getTopStarredWeblogs(0, length);
        } catch (Exception e) {
