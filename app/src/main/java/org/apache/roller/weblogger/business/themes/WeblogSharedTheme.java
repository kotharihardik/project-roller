 ...
 * Uses composition instead of inheritance to avoid coupling theme logic with content ownership.
 */
public class WeblogSharedTheme implements Theme {

    private static Log log = LogFactory.getLog(WeblogSharedTheme.class);

    private final SharedTheme baseTheme;
    ...
    public String getDescription() {
        return baseTheme.getDescription();
    }

    @Override
    public Date getLastModified() {
        return baseTheme.getLastModified();
    }

    @Override
    public boolean isEnabled() {
        return baseTheme.isEnabled();
    }

    @Override
    public int compareTo(Theme other) {
        return baseTheme.compareTo(other);
    }
    ...
    public List<ThemeTemplate> getTemplates() throws WebloggerException {
        ...
        // first get the pages from the db (these are WeblogTemplate objects)
        try {
        ...
        try {
            for (ThemeTemplate template : this.baseTheme.getTemplates()) {
    ...
    @Override
     public ThemeTemplate getStylesheet() throws WebloggerException {
        ...
        ThemeTemplate stylesheet = this.baseTheme.getStylesheet();
        if (stylesheet != null) {
    ...
    public ThemeTemplate getDefaultTemplate() throws WebloggerException {
        return this.baseTheme.getDefaultTemplate();
    }
    ...
    public ThemeTemplate getTemplateByAction(ComponentType action) throws WebloggerException {
        ...
        return this.baseTheme.getTemplateByAction(action);
    }
    ...
    public ThemeTemplate getTemplateByName(String name) throws WebloggerException {
        ...
        template = this.baseTheme.getTemplateByName(name);
    ...
    @Override
     public ThemeTemplate getTemplateByLink(String link) throws WebloggerException {
        ...
        // first check if this user has selected a theme
        // if so then return the proper theme template
        template = this.baseTheme.getTemplateByLink(link);
    ...
    public ThemeResource getResource(String path) {
        ...
        resource = this.baseTheme.getResource(path);
