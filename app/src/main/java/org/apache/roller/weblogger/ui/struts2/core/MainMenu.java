...
public class MainMenu extends UIAction {
    ...
    public List<StarredWeblogView> getStarredWeblogs() {
        try {
            StarService service = WebloggerFactory.getWeblogger().getStarService();
            return service.getStarredWeblogsSortedByLatestPost(getAuthenticatedUser().getId(), 0, -1);
        } catch (Exception e) {
            log.error("Error loading starred weblogs for user", e);
            return Collections.emptyList();
        }
    }
}
