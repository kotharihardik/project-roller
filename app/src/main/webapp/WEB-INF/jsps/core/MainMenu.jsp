...
<s:if test="!starredWeblogs.isEmpty()">
    <h3><s:text name="yourWebsites.starredWeblogs" /></h3>
    <ul>
        <s:iterator value="starredWeblogs" var="sw">
            <li>
                <a href="<s:property value='#sw.weblog.absoluteURL'/>">
                    <s:property value="#sw.weblog.name"/>
                </a>
                -
                <s:text name="yourWebsites.lastPostedAt"/>
                <s:if test="#sw.lastPublishedPostTime != null">
                    <s:date name="#sw.lastPublishedPostTime" format="yyyy-MM-dd HH:mm"/>
                </s:if>
                <s:else>
                    <s:text name="yourWebsites.noPostsYet"/>
                </s:else>
            </li>
        </s:iterator>
    </ul>
